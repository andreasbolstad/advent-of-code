...
cardinal_directions = [
    np.linalg.matrix_power(R_90, i) @ cardinal_start for i in range(4)
]

diagonal_start = np.array([1, 1])  # Northeast
diagonal_directions = [
    np.linalg.matrix_power(R_90, i) @ diagonal_start for i in range(4)
]
...
start_positions = [
    (x, y) for x in range(nx) for y in range(ny) if is_start_of_word(x, y)
]
...
is_word = lambda x, y, d: all(
    grid[x + i * d[0]][y + i * d[1]] == w[i] for i in range(1, wl)
)
count_words = sum(
    is_within_bounds(x, y, d) and is_word(x, y, d)
